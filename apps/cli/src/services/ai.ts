...
class AIService {
  async analyzeGitActivity(): Promise<CodeActivity> {
    ...
    try {
      // Get recent commits
      const commitsData = await this.execGit([
        "log",
        "--oneline",
        "--since=1 hour ago", // no quotes around value — shell:false doesn't need them
      ]);
      ...
      diffLines.forEach((line) => {
        ...
        if (addMatch) activity.linesAdded = parseInt(addMatch[1]);

        const delMatch = line.match(/(\d+) deletions?/);
        if (delMatch) activity.linesDeleted = parseInt(delMatch[1]);
      });
      ...
      // Check for debugging patterns in last commit message
  ...
  // shell: false — prevents command injection via branch names or filenames
  private execGit(args: string[]): Promise<string> {
    return new Promise((resolve, reject) => {
      // Validate args: only allow safe characters
      const safe = args.every((a) => /^[a-zA-Z0-9@~_.:\-/ ]+$/.test(a));
      if (!safe) {
        return reject(new Error("Git args contain unsafe characters"));
      }

      const git = spawn("git", args, { shell: false }); // shell: false
      let output = "";
      let errOutput = "";

      git.stdout?.on("data", (data) => { output += data.toString(); });
      git.stderr?.on("data", (data) => { errOutput += data.toString(); });

      git.on("error", (err) => reject(err));
      git.on("close", (code) => {
        if (code === 0) resolve(output.trim());
        else reject(new Error(`Git command failed (${code}): ${errOutput.trim()}`));
      });
    });
  }
